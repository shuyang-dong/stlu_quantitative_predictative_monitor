...
behavior_type_list = [0, 2] # 0:cautious, 2:aggressive
segment_folder_LQT = '/home/cpsgroup/predictive_monitor_new_stl/pre_alert_time_evaluation_lqt'
b0_path_LQT = '{segment_folder}/b0_lqt/segment_results_b_1024_seqlen_50_stepback_30_e_1_lr_0.001_f_17_conf_0.95_dt_1_dr_0.8.csv'.format(segment_folder=segment_folder_LQT)
b2_path_LQT = '{segment_folder}/b2_lqt/segment_results_b_1024_seqlen_50_stepback_30_e_1_lr_0.001_f_17_conf_0.95_dt_4_dr_0.8.csv'.format(segment_folder=segment_folder_LQT)
path_dict_LQT={0:b0_path_LQT, 2:b2_path_LQT}
col = ['behavior_type','hazard_type', 'method','metric_type','TP', 'TN', 'FP', 'FN', 'TPR', 'TNR', 'FPR', 'FNR',
...
for behavior_type in behavior_type_list:
  ...
  segment_file_path = path_dict_LQT[behavior_type]
  for hazard_type in hazard_type_list:
    ...
    for method in method_list:
      ...
      normal_results_dict['behavior_type']=behavior_type
      consecutive_results_dict['behavior_type']=behavior_type
...
def mark_hazard_id_for_real_acc(driving_trace_df, low=-6.0, high=6.0):
  ...
  hazard_type_list = []
  hazard_time_list = []
  acc_list = driving_trace_df['real_acc'].tolist()
  hazard_id_count = 0
  hazard_type = 0
  hazard_time = 0
  ...
  for i in range(len(acc_list)):
    ...
    if i==0:
      ...
      if hazard_type!=last_hazard_type and hazard_type!=0:
        if i-last_hazard_end_time<=10: # within 30min # within 1.0s
          hazard_id_list.append(0)
          last_hazard_end_time = i
        else:
          hazard_id_count += 1
  ...
  for j in range(len(acc_list)):
    if j in hazard_id_index_list:
      next_index = hazard_id_index_list[hazard_id_index_list.index(j)+1] if hazard_id_index_list.index(j)!=(len(hazard_id_index_list)-1) else len(acc_list)-1
      if j != next_index:
          ...
          max_acc_this_hazard = max(driving_trace_df[j:next_index][driving_trace_df['hazard_type']!=0]['real_acc'].to_list())
          min_acc_this_hazard = min(driving_trace_df[j:next_index][driving_trace_df['hazard_type']!=0]['real_acc'].to_list())
          high_time_this_hazard = len(driving_trace_df[j:next_index][(driving_trace_df['hazard_type']!=0) & (driving_trace_df['real_acc']>high)])
          low_time_this_hazard = len(driving_trace_df[j:next_index][(driving_trace_df['hazard_type']!=0) & (driving_trace_df['real_acc']<low*0.5)])
...
def calculate_pre_alert_time(segment_file_path, real_acc_trace_path, trace_start_index, trace_end_index,
  ...
  if hazard_type=='total':
    ...
    hazard_df = real_acc_df[(real_acc_df['hazard_id']!=0) & ((real_acc_df['final_hazard_type']==3) | (real_acc_df['final_hazard_type']==5))]
    total_hazard_num = len(hazard_df)
    total_pre_alert_time = sum(hazard_df['pre_alert_time'].to_list())
  elif hazard_type=='low':
    hazard_df = real_acc_df[(real_acc_df['hazard_id']!=0) & ((real_acc_df['final_hazard_type']==2) | (real_acc_df['final_hazard_type']==4))]
    total_hazard_num = len(hazard_df)
    total_pre_alert_time = sum(hazard_df['pre_alert_time'].to_list())
  ...
  print('control type: ', control_type, ' total_hazard_num: ', total_hazard_num, ' average_pre_alert_time step: ', average_pre_alert_time)
...
control_type_list = ['lstm_with_monitor','lstm_no_monitor']#'lstm_with_monitor',
hazard_type_list = ['total', 'high', 'low']

# calculate the average pre alert time and total hazard num for each trace for each behavior type & control type & hazard type
all_pre_alert_time_path = '{folder}/pre_alert_time_summary_LQT_each_trace.csv'.format(folder=segment_folder_LQT)
all_pre_alert_time_df = pd.DataFrame(columns=['behavior_type','control_type', 'hazard_type' ,
                                                'seed', 'episode', 'trace_len', 'start_index', 'end_index',
                                              'total_hazard_num', 'average_pre_alert_time_step', 'average_pre_alert_time_second'])
####
for behavior_type in behavior_type_list:
  for control_type in control_type_list:
    for hazard_type in hazard_type_list:
      print('behavior_type: ', behavior_type, ' control_type: ', control_type, ' hazard_type: ', hazard_type)
      real_acc_trace_path = '{folder}/real_acc_trace_with_hazard_behavior_type_{behavior_type}_{control_type}.csv'.format(behavior_type=behavior_type, folder=segment_folder_LQT, control_type=control_type)
      segment_file_path = path_dict_LQT[behavior_type]
      df = pd.read_csv(segment_file_path)
      print(len(df))
      # get each trace in the segment file by seed and episode separately and calculate each metrics value
      total_seed_info_file = '{segment_folder_LQT}/trace_seed_episode_step_info_test_set_behavior_{behavior_type}.csv'.format(segment_folder_LQT=segment_folder_LQT,behavior_type=behavior_type)
      trace_seed_info_df = pd.read_csv(total_seed_info_file)
      total_seed_list = trace_seed_info_df['seed'].to_list()
      total_seed_list = list(set(total_seed_list))
      total_seed_list.sort()
      print('total_seed_list: ', total_seed_list)
      trace_start_index = 0
      trace_end_index = 0
      for seed in total_seed_list:
          total_episode_list = trace_seed_info_df[trace_seed_info_df['seed']==seed]['episode']
          total_episode_list = list(set(total_episode_list))
          total_episode_list.sort()
          for episode in total_episode_list:
              trace_info_df = trace_seed_info_df[(trace_seed_info_df['seed']==seed) & (trace_seed_info_df['episode']==episode)]
              trace_len = len(trace_info_df)
              trace_end_index = trace_start_index + trace_len - 1
              print('seed: ', seed, ' episode: ', episode, ' trace len:', trace_len, ' start: ', trace_start_index, ' end: ', trace_end_index)
              total_hazard_num, average_pre_alert_time_step = calculate_pre_alert_time(segment_file_path, real_acc_trace_path, trace_start_index, trace_end_index, control_type=control_type, hazard_type=hazard_type)
              average_pre_alert_time_second = average_pre_alert_time_step*0.1
              result_dict = {'seed':seed, 'episode':episode, 'trace_len': trace_len, 'start_index':trace_start_index, 'end_index':trace_end_index,
                  'behavior_type':behavior_type,'control_type':control_type, 'hazard_type':hazard_type,'total_hazard_num':total_hazard_num,
                            'average_pre_alert_time_step':average_pre_alert_time_step, 'average_pre_alert_time_second':average_pre_alert_time_second}
              all_pre_alert_time_df = all_pre_alert_time_df.append(result_dict, ignore_index=True)
              trace_start_index = trace_end_index+1
all_pre_alert_time_df.to_csv(all_pre_alert_time_path)
####"""

# calculate the average pre alert time and total hazard num among all traces for each behavior type  & control type & hazard type
total_trace_result_df = pd.DataFrame(columns=['behavior_type','control_type', 'hazard_type',
                                   'total_hazard_num_all_trace',
                                    'average_pre_alert_time_step_all_trace',
                                   'average_pre_alert_time_second_all_trace'])
total_trace_result_path = '{segment_folder_LQT}/pre_alert_time_summary_LQT_all_trace.csv'.format(segment_folder_LQT=segment_folder_LQT)
all_pre_alert_time_df = pd.read_csv(all_pre_alert_time_path, index_col=0)
all_pre_alert_time_df=all_pre_alert_time_df[all_pre_alert_time_df['average_pre_alert_time_step']>=0]
#print('all_pre_alert_time_df: ', all_pre_alert_time_df)
for behavior_type in behavior_type_list:
  df_b = all_pre_alert_time_df[all_pre_alert_time_df['behavior_type']==behavior_type]
  #print(df_b)
  for control_type in control_type_list:
    df_bc = all_pre_alert_time_df[(all_pre_alert_time_df['behavior_type'] == behavior_type)&(all_pre_alert_time_df['control_type'] == control_type)]
    #print(df_bc)
    for hazard_type in hazard_type_list:
        print('behavior_type: ', behavior_type, ' control_type: ', control_type, ' hazard_type: ', hazard_type)
        df = df_bc[df_bc['hazard_type']==hazard_type]
        #df = all_pre_alert_time_df[(all_pre_alert_time_df['behavior_type']==behavior_type) & (all_pre_alert_time_df['control_type']==control_type) & all_pre_alert_time_df['hazard_type']==hazard_type]
        print('df: ', df)
        average_pre_alert_time_step_all_trace = df['average_pre_alert_time_step'].mean()
        average_pre_alert_time_second_all_trace = df['average_pre_alert_time_second'].mean()
        total_hazard_num_all_trace = df['total_hazard_num'].mean()
        total_trace_result_dict = {'behavior_type':behavior_type,'control_type':control_type, 'hazard_type':hazard_type,
                                   'total_hazard_num_all_trace':total_hazard_num_all_trace,
                                    'average_pre_alert_time_step_all_trace':average_pre_alert_time_step_all_trace,
                                   'average_pre_alert_time_second_all_trace':average_pre_alert_time_second_all_trace}
        print('total_trace_result_dict: ', total_trace_result_dict)
        total_trace_result_df= total_trace_result_df.append(total_trace_result_dict, ignore_index=True)
total_trace_result_df.to_csv(total_trace_result_path)
##"""
