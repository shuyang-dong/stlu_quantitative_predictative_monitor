...
random.shuffle(trace_seed_list)
print('trace_seed_list: ',trace_seed_list)
...
def generate_dataset_by_type(trace_folder, behavior_type, route_id_list, episode_num, scenario_id,
  ...
  for dataset_single_trace in new_df_list:
    for i in range(len(dataset_single_trace)-seq_length):
...
train_sample_x = all_dataX[:int(len(all_dataX) * 0.80)]  # 70% for training
train_sample_y = all_dataY[:int(len(all_dataY) * 0.80)]
test_sample_x = all_dataX[int(len(all_dataX) * 0.80):int(len(all_dataX) * 0.90)]  # 15% for test
test_sample_y = all_dataY[int(len(all_dataY) * 0.80):int(len(all_dataY) * 0.90)]
valid_sample_x = all_dataX[int(len(all_dataX) * 0.90):]  # 15% for validation
...
all_mean_value_df = mean_value_1
all_std_value_df = std_value_1
all_mean_value_df.columns=[behavior_type]
all_std_value_df.columns=[behavior_type]
save_folder = '{CHECKPOINT_PATH}/with_dropout_new_stl'.format(CHECKPOINT_PATH=CHECKPOINT_PATH)
mkdir(save_folder)
all_mean_path = '{save_folder}/all_mean_value_{behavior_type}.csv'.format(save_folder=save_folder, behavior_type=behavior_type)
all_std_path = '{save_folder}/all_std_value_{behavior_type}.csv'.format(save_folder=save_folder,behavior_type=behavior_type)
...
def requirement_func_always_speed_in_range(signal, trace_len, conf, lower_speed=2.0, upper_speed=100.0, func='monitor'):
...
num_epochs = 700
num_epochs_list = [100, 150, 200, 250, 300, 350, 400, 500, 600, 700]
...
output_folder = '/home/cpsgroup/predictive_monitor_new_stl/with_dropout_new_stl/select_dropout_with_lossfunc_new/results/behavior_type_{behavior_type}_f_{feature_num}'.format(
...
for learning_rate in learning_rate_list:
    for conf in conf_list:
        ...
        for epoch in range(num_epochs):
            ...
            test_loss = test_model(model, test_loader, criterion, device, predict_length, N_MC, batch_size, hidden_size,
            ...
            if (epoch + 1) in num_epochs_list:
                ...
                loss_one_sample_average = save_results(model, epoch + 1, train_loss, test_loss,
                                                       requirement_func_always_acc_in_range,
                ...
                model_name = 'lstm_s1_with_dropout_lr_{learning_rate}_dt_{dropout_type}_dr_{dropout_rate}_e_{e}_behavior_type_{behavior_type}_f_{feature_num}'.format(
                    learning_rate=learning_rate,
                    dropout_type=dropout_type_for_train, dropout_rate=dropout_rate_for_train, e=epoch + 1,
                    behavior_type=behavior_type,
                    feature_num=selected_feature_num)
