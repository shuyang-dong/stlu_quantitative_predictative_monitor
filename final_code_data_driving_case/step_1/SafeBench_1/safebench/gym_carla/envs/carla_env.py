...
class CarlaEnv(gym.Env):
    ...
    def _create_sensors(self):
        # obstacle sensor
        self.obs_bp = self.world.get_blueprint_library().find('sensor.other.obstacle')
        self.obs_bp.set_attribute("only_dynamics", 'True')
        self.obs_bp.set_attribute('sensor_tick', '0.02')
        self.obs_bp.set_attribute('debug_linetrace', 'True')
        self.obs_bp.set_attribute('distance', '1')
        self.obs_bp.set_attribute('hit_radius', '0.5')

        # radar sensor
        self.radar_object_df = pandas.DataFrame()
        self.rad_bp = self.world.get_blueprint_library().find('sensor.other.radar')
        self.rad_bp.set_attribute('horizontal_fov', str(35))
        self.rad_bp.set_attribute('vertical_fov', str(20))
        self.rad_bp.set_attribute('range', str(30))
    ...
    def _parse_route(self, config):
        ...
        for node in route:
            ...
            #print('waypoint: ', type(waypoint), waypoint.transform)
    ...
    def _attach_sensor(self):
        ...
        self.obstacle_sensor = self.world.spawn_actor(self.obs_bp, obs_transform, attach_to=self.ego_vehicle,
                                    attachment_type=carla.AttachmentType.Rigid)
        def obs_callback(obs):
            #print("Obstacle detected: " + str(obs), " type: ", obs.other_actor.type_id, ' distance: ', obs.distance)
            #print('Other actor type: ', obs.other_actor.type_id)
            #print('Other actor distance: ', obs.distance)
            return 0
        ...
        self.radar_sensor = self.world.spawn_actor(self.rad_bp, rad_transform, attach_to=self.ego_vehicle,
                                           attachment_type=carla.AttachmentType.Rigid)

        def rad_callback(radar_data):
            #print('radar data: ', type(radar_data))
            velocity_range = 7.5  # m/s
            current_rot = radar_data.transform.rotation
            current_loc = radar_data.transform.location
            self.radar_object_df=pandas.DataFrame(columns=['azi', 'alt', 'velocity', 'depth'])
            for detect in radar_data:
                ...
                fw_vec_0 = carla.Vector3D(x=current_loc.x - 20, y=current_loc.y, z=current_loc.z)
                carla.Transform(
                    carla.Location(),
                    carla.Rotation(
                ...
                # Recasting a ray from sensor to detected location, but getting object this time
                #hit_point = self.world.project_point(current_loc, self.ego_vehicle.get_velocity(), detect.depth + 50)
                #print('hit_point: ', hit_point.label, hit_point.location)
                # store data
                #obj_attribute = {'azi':azi, 'alt':alt, 'velocity':detect.velocity, 'depth':detect.depth}
                # print('radar obj_attribute: ', obj_attribute)
                # print('ego veh location: ', self.ego_vehicle.get_transform())
                #cast_ray_point = self.world.cast_ray(current_loc, fw_vec_0)
                #for o in cast_ray_point:
                #    if str(o.label) == 'Vehicles':
                #       print('cast_ray_point: ', o.label, o.location)
                # print('cast_ray_point: ', cast_ray_point)
                # vehicle_info_dict_list = self._get_actor_info('vehicle.*')[0]
                # for k in vehicle_info_dict_list.keys():
                #     #print('vehicle_info_dict_list: ', vehicle_info_dict_list[k].x, vehicle_info_dict_list[k].y, vehicle_info_dict_list[k].z)
                #     print('vehicle_info_dict_list location: ', vehicle_info_dict_list[k])
                # vehicle_info_dict_list_1 = self._get_actor_info('vehicle.*')[3]
                # for k in vehicle_info_dict_list.keys():
                #     # print('vehicle_info_dict_list: ', vehicle_info_dict_list[k].x, vehicle_info_dict_list[k].y, vehicle_info_dict_list[k].z)
                #     print('vehicle_info_dict_list speed: ', vehicle_info_dict_list_1[k])
                if detect.depth>3.0: # avoid detect ego vehicle
                    obj_attribute = {'azi': azi, 'alt': alt, 'velocity': detect.velocity, 'depth': detect.depth}
                    self.radar_object_df = self.radar_object_df.append(obj_attribute, ignore_index=True)
            ...
            # print('ALL radar data: ', self.radar_object_df)
            # radar_obj_depth_aveg = self.radar_object_df['depth'].mean()
            # radar_obj_depth_std = self.radar_object_df['depth'].std()
            # radar_obj_velocity_aveg = self.radar_object_df['velocity'].mean()
            # radar_obj_velocity_std = self.radar_object_df['velocity'].std()
            # print('average depth: ', radar_obj_depth_aveg, ' depth std: ', radar_obj_depth_std,
            #       ' average velocity: ', radar_obj_velocity_aveg,
            #       ' velocity std: ', radar_obj_velocity_std)
            # print('ego veh location: ', self.ego_vehicle.get_transform())
            # vehicle_info_dict_list, _,_,vehicle_info_dict_list_1 = self._get_actor_info('vehicle.*')
            # for k in vehicle_info_dict_list.keys():
            #     # print('vehicle_info_dict_list: ', vehicle_info_dict_list[k].x, vehicle_info_dict_list[k].y, vehicle_info_dict_list[k].z)
            #     print('vehicle_info_dict_list location: ', vehicle_info_dict_list[k])
            # #vehicle_info_dict_list_1 = self._get_actor_info('vehicle.*')
            # for k in vehicle_info_dict_list_1.keys():
            #     # print('vehicle_info_dict_list: ', vehicle_info_dict_list[k].x, vehicle_info_dict_list[k].y, vehicle_info_dict_list[k].z)
            #     print('vehicle_info_dict_list speed: ', vehicle_info_dict_lis
        self.radar_sensor.listen(lambda radar_data: rad_callback(radar_data))
    ...
    def _get_actor_info(self, filt):
        ...
        for actor in self.world.get_actors().filter(filt):
            #print('get actor: ', actor, actor.id, actor.type_id)
    ...
    def _get_obs(self):
        ...
        #print('ego_trans: ', ego_trans)
        #print('ego v: ', v, speed)
        # add radar info
        radar_obj_depth_aveg = -1
        radar_obj_depth_std = -1
        radar_obj_velocity_aveg = -1
        radar_obj_velocity_std = -1
        if len(self.radar_object_df)>0:
            # clean outlier data points, mainly the vehicle from another direction
            radar_obj_velocity_aveg = self.radar_object_df['velocity'].mean()
            radar_obj_velocity_std = self.radar_object_df['velocity'].std()
            up_velocity = radar_obj_velocity_aveg + 2 * radar_obj_velocity_std
            down_velocity = radar_obj_velocity_aveg - 2 * radar_obj_velocity_std
            self.radar_object_df = self.radar_object_df.drop(
                self.radar_object_df[(self.radar_object_df['velocity'] < down_velocity) | (self.radar_object_df['velocity'] > up_velocity)].index)
            # print('self.radar_object_df: ', self.radar_object_df)
            radar_obj_depth_aveg = self.radar_object_df['depth'].mean()
            radar_obj_depth_std = self.radar_object_df['depth'].std()
            radar_obj_velocity_aveg = self.radar_object_df['velocity'].mean()
            radar_obj_velocity_std = self.radar_object_df['velocity'].std()
        ...
        state = np.array([lateral_dis, -delta_yaw, speed, self.vehicle_front,
                          ...
                          ego_x, ego_y, ego_yaw,
                          v.x, v.y,
                          radar_obj_depth_aveg, radar_obj_depth_std, radar_obj_velocity_aveg, radar_obj_velocity_std])
        #print('state in carla env: ', state)
