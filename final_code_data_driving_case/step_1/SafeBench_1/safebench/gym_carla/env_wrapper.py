...
class ObservationWrapper(gym.Wrapper):
    ...
    def _preprocess_obs(self, obs):
        ...
        if self.obs_type == 0:
            return obs['state'][:16].astype(np.float64)
        # concat the 4-dimensional state space and lane info
        elif self.obs_type == 1:
            new_obs = np.array([
                obs['state'][0], obs['state'][1], obs['state'][2], obs['state'][3],
                obs['state'][4], obs['state'][5], obs['state'][6], obs['state'][7],
                obs['state'][8], obs['state'][9], obs['state'][10], obs['state'][11],
                obs['state'][12], obs['state'][13], obs['state'][14], obs['state'][15],
                obs['command'], 
        ...
        # return a dictionary with bird-eye view image and state
            ...
            return {"img": obs['birdeye'], "states": obs['state'][:16].astype(np.float64)}
        # return a dictionary with front-view image and state
        elif self.obs_type == 3:
            return {"img": obs['camera'], "states": obs['state'][:16].astype(np.float64), "lidar":obs['lidar']}
        else:
            raise NotImplementedError
