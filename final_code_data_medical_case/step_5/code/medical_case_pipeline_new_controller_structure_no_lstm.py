...
class NewController(Controller):
    ...
    def policy(self, past_trace_df, current_rho_set_CGM, current_pred_CGM_mean, current_pred_CGM_std,
        ...
        if current_state == 4:  # servere low violation, take new action immediately
            ...
            if past_basal_severe_low_adjust_list.count(1)!=0:
                ...
                action, (basal, correction_bolus, meal_bolus,max_total_bolus_trigger_flag) = self._new_policy(pname, meal, current_CGM, sample_time,past_trace_df,current_time,
    ...
    def _new_policy(self, name, meal, glucose, env_sample_time, past_trace_df, current_time, bg_type='low'):
        ...
        # give correction bolus before meal
        meal_time_list = self.meal_time_list_df[name].to_list()
        future_time = current_time + timedelta(minutes=self.arg_correction_bolus_ahead_step*3)
        future_time_str = future_time.strftime("%Y-%m-%d %H:%M:%S")
        if future_time_str in meal_time_list:
            insulinReq = ((glucose.item() - 70) / (quest.CF.values)).item()
            bolus = (glucose.item() > 70) * (self.correction_bolus_parameter * insulinReq / env_sample_time)
        else:
            bolus = 0
        ...
        if meal > 0:
            ...
            logger.info(f'glucose = {glucose.item()}')
            # print('glucose in new policy: ', type(glucose.item()), glucose.item())
            # print('self.target in new policy: ', type(self.target), self.target)
            meal_bolus = (
                    (meal * env_sample_time) / quest.CR.values + (glucose.item() > 150) *
    ...
    def _bb_policy(self, name, meal, glucose, env_sample_time, current_state_from_main_controller):
        ...
        if meal > 0:
            ...
            #print('glucose.CGM: ', type(glucose.CGM), glucose.CGM)
            #print('self.target: ', type(self.target), self.target)
            bolus = (
...
all_meal_time_df = pd.DataFrame()
all_meal_amount_df = pd.DataFrame()
for patient_type in patient_type_list:
  for patient_id in patient_id_num_list: 
    ...
    for lstm_type in lstm_type_list:
          ...
          simglucose_test_results_path = '/Medical_case/medical_case_pipeline/results/{lstm_type}'.format(lstm_type=lstm_type)
          mkdir(simglucose_test_results_path)

          # specify start_time as the beginning of today
          now = datetime(2022, 10, 3)#datetime.now()+timedelta(days=)
          print('date now: ', now)
