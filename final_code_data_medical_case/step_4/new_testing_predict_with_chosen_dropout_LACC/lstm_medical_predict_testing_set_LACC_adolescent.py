...
def test(lstm_model, test_loader, criterion, device, predict_length, N_MC,batch_size,hidden_size,dropout_rate,dropout_type,step_look_back):
  ...
  test_loss_mean = 0
  with torch.no_grad():
      for batch_idx, datasample in enumerate(test_loader):  #tqdm(enumerate(test_loader)):
          data,target0 = datasample[0], datasample[1]
          target = []
          for target_data in datasample[1]:
            target.append(target_data[step_look_back:,:])
...
def save_results(lstm_model, num_epochs, train_loss, valid_loss, requirement_func, dropout_rate_for_test,
    ...
    get_if_warning_label_each_segment_one_side_STLU(segment_output_file_path)
    results_dict_one_side = calculate_lane_lable_metrics_one_side_STLU(segment_output_file_path)
...
def get_if_warning_label_each_segment_one_side_STLU(segment_output_file_path):
...
def calculate_lane_lable_metrics_one_side_STLU(segment_output_file_path):
    segment_df = pd.read_csv(segment_output_file_path, index_col=0)
...
output_folder = '/Medical_case/with_dropout/new_testing_predict_with_chosen_dropout_LACC/results/{p_type}_patient'.format(p_type=p_type)
mkdir(output_folder)
...
dropout_rate_dict = {'adult':0.4, 'child':0.4, 'adolescent':0.5}
dropout_type_dict = {'adult':4, 'child':3, 'adolescent':3}
