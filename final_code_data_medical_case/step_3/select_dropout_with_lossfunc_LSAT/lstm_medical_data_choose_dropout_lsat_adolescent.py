...
# Path to the folder where the pre-trained models are saved
p_type = 'adolescent'
...
def train(lstm_model, train_loader, criterion, optimizer, device, batch_size, hidden_size, dropout_rate, step_look_back):
  ...
  lstm_model.train()
  train_loss = 0
  for batch_idx, datasample in enumerate(train_loader): #tqdm(enumerate(train_loader)):
...
def lsat(trace_uncertain, trace_origin, requirement, beta_1=0.25, beta_2=0.25):
...
def eval_model(device, segment_result_col, dataset_loader, lstm_model, criterion,
    ...
    with torch.no_grad():
        for batch_idx, datasample in enumerate(dataset_loader):
            ...
            loss_L_SAT += lsat(trace_CGM[:, :, :-2], trace_CGM[:, :, -2:], requirement_func)
            # record mean and std value for each sample after applying dropout and loop N_MC times
            for b in range(batch_len):
                # calcullate real lane width of one sample trace in the batch
...
# evaluation with STL-U and dropout: using Lsat
...
def save_results(lstm_model, num_epochs, train_loss, valid_loss, requirement_func, dropout_rate_for_train,
    ...
    result_col = ['selected_feature_col', 'seq_length', 'step_look_back', 'predict_len', 'conf',
                  ...
                  'valid_loss',
                  'test_loss', 'loss_one_sample_average', 'loss_L_SAT']
    ...
    loss_one_sample_average, loss_L_SAT, test_loss, test_loss_mean, segment_result_df = eval_model(device, segment_result_col,
    ...
    result_dict = {'type': 'with_dropout',
                   ...
                   'test_loss': test_loss, 'loss_one_sample_average': loss_one_sample_average, 'loss_L_SAT':loss_L_SAT,
    ...
    return loss_one_sample_average, loss_L_SAT


# STL formula for requirement on CGM
def requirement_func_always_BG_in_range(signal, trace_len=10, conf=0.95, lower_BG=70, upper_BG=180, func='monitor'):
    # signal: CGM trace
    # Keep BG in range
...
# funcs for saving models-not used
...
num_epochs = 1
num_epochs_list = [1]
learning_rate_list = [0.001] # no use, just for unify output file name
...
step_look_back=10
predict_length=10
# list for choosing dropout type/rate params
dropout_type_list = [4, 3, 2, 1]
dropout_rate_list = [0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1]

output_folder = '/Medical_case/with_dropout/select_dropout_with_lossfunc_LSAT/results/{month}_m_{p_type}_patient_{feature_num}_f'.format(
...
# adult model
adult_lstm_path = '/Medical_case/with_dropout/new_testing_predict_with_chosen_dropout/trained_model/lstm_s1_with_dropout_lr_0.01_dt_4_dr_0.9_e_50_3_m_8_f_adult_patient_state_dict.pt'
# child model
child_lstm_path = '/Medical_case/with_dropout/new_testing_predict_with_chosen_dropout/trained_model/lstm_s1_with_dropout_lr_0.01_dt_4_dr_0.9_e_50_3_m_8_f_child_patient_state_dict.pt'
# adolescent model
adolescent_lstm_path = '/Medical_case/with_dropout/new_testing_predict_with_chosen_dropout/trained_model/lstm_s1_with_dropout_lr_0.001_dt_4_dr_0.9_e_50_3_m_8_f_adolescent_patient_state_dict.pt'
            ...
            if (epoch + 1) in num_epochs_list:
                ...
                for dropout_type in dropout_type_list:
                    for dropout_rate in dropout_rate_list:
                        loss_one_sample_average, loss_L_SAT, test_loss, test_loss_mean = eval_model_new_loss(device, valid_loader, with_dropout_model,
                                                                                                 criterion,
                                                                                                 seq_length, step_look_back,
                                                                                                 learning_rate, N_MC,
                                                                                                 batch_size, hidden_size,
