...
def preprocess_medical_data(patient_data_file):
  # get time, hour and minute separately
...
month = 3
data_folder = '/Medical_case/data/patient_data_{month}_m'.format(month=month)
DATASET_PATH = data_folder
# Path to the folder where the pretrained models are saved
p_type = 'adolescent'
CHECKPOINT_PATH = "/Medical_case/with_dropout/select_dropout_with_lossfunc_new/results/new_saved_models/{month}_m_{p_type}_patient".format(month=month, p_type=p_type)
...
# save the mean and std values for converting standardlized data back to original scale
all_mean_value_df = pd.concat([mean_value_1, mean_value_2, mean_value_3], axis=1)
all_std_value_df = pd.concat([std_value_1, std_value_2, std_value_3], axis=1)
all_mean_value_df.columns=['adult', 'child', 'adolescent']
all_std_value_df.columns=['adult', 'child', 'adolescent']
save_folder = '/content/drive/MyDrive/ColabNotebooks/Medical_case/with_dropout'
all_mean_path = '{save_folder}/all_mean_value.csv'.format(save_folder=save_folder)
all_std_path = '{save_folder}/all_std_value.csv'.format(save_folder=save_folder)
all_mean_value_df.to_csv(all_mean_path)
all_std_value_df.to_csv(all_std_path)
...
def save_results(lstm_model, num_epochs, train_loss, valid_loss, requirement_func, dropout_rate_for_train,
                 dropout_type_for_train, conf, learning_rate, output_folder, mean_value_df, std_value_df):
    ...
    result_output_path = '{output_folder}/results_b_{batch_size}_seqlen_{seq_length}_stepback_{step_look_back}_e_{num_epochs}_lr_{learning_rate}_f_{selected_feature_num}_conf_{conf}_dt_{dropout_type}_dr_{dropout_rate}.csv'.format(
        ...
        learning_rate=learning_rate, selected_feature_num=selected_feature_num, dropout_type=dropout_type_for_train,
        dropout_rate=dropout_rate_for_train, conf=conf)
    ...
    segment_output_file_path = '{output_folder}/segment_results_b_{batch_size}_seqlen_{seq_length}_stepback_{step_look_back}_e_{num_epochs}_lr_{learning_rate}_f_{selected_feature_num}_conf_{conf}_dt_{dropout_type}_dr_{dropout_rate}.csv'.format(
        ...
        dropout_type=dropout_type_for_train, dropout_rate=dropout_rate_for_train, conf=conf)
    ...
    loss_one_sample_average, test_loss, test_loss_mean, segment_result_df = eval_model(device, segment_result_col,
                                                                                       ...
                                                                                       dropout_rate_for_train,
                                                                                       dropout_type_for_train,
    ...
    result_dict = {'type': 'with_dropout',
                   ...
                   'N_MC': N_MC, 'dropout_type': dropout_type_for_train, 'dropout_rate': dropout_rate_for_train,
    ...
    results_one_side_output_path = '{output_folder}/metrics_results_one_side_b_{batch_size}_seqlen_{seq_length}_stepback_{step_look_back}_e_{num_epochs}_lr_{learning_rate}_f_{selected_feature_num}_conf_{conf}_dt_{dropout_type}_dr_{dropout_rate}.csv'.format(
        output_folder=output_folder, batch_size=batch_size, seq_length=seq_length, step_look_back=step_look_back,
        num_epochs=epoch, learning_rate=learning_rate, selected_feature_num=selected_feature_num,
        dropout_type=dropout_type_for_train, dropout_rate=dropout_rate_for_train, conf=conf)
...
def requirement_func_always_BG_in_range(signal, trace_len, conf, lower_BG=70, upper_BG=180, func='monitor'):
    # signal: CGM trace, Keep CGM in range
...
output_folder = '/Medical_case/with_dropout/select_dropout_with_lossfunc_new/results/{month}_m_{p_type}_patient_{feature_num}_f'.format(
    month=month, feature_num=selected_feature_num, p_type=p_type)
mkdir(output_folder)
print('output_folder: ', output_folder)
dropout_choice_df = pd.DataFrame(columns=['dropout_type', 'dropout_rate', 'loss_value'])

# specify model parameters and train
for learning_rate in learning_rate_list:
    for conf in conf_list:
        # specify model parameters and train
        model = LSTM(num_classes, input_size, hidden_size, num_layers, step_look_back, dropout_type_for_train)
        model = model.to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
        for epoch in range(num_epochs):
            train_loss = train(model, train_loader, criterion, optimizer, device, batch_size, hidden_size,
                               dropout_rate_for_train, step_look_back)
            test_loss = test(model, test_loader, criterion, device, predict_length, N_MC, batch_size, hidden_size,
                             dropout_rate_for_train, dropout_type_for_train, step_look_back)
            if (epoch + 1) % 10 == 0:
                print(f"[Epoch {epoch + 1:2d}] Training loss: {train_loss}, Test loss: {test_loss}")
            if (epoch + 1) in num_epochs_list:
                print(f"[Epoch {epoch + 1:2d}] Training loss: {train_loss}, Test loss: {test_loss}")
                loss_one_sample_average = save_results(model, epoch + 1, train_loss, test_loss,
                                                       requirement_func_always_BG_in_range,
                                                       dropout_rate_for_train, dropout_type_for_train,
                                                       conf, learning_rate, output_folder, mean_value, std_value)
                print(f"[Epoch {epoch + 1:2d}] Validation loss: {loss_one_sample_average}")
                model_name = 'lstm_s1_with_dropout_lr_{learning_rate}_dt_{dropout_type}_dr_{dropout_rate}_e_{e}_{month}_m_{feature_num}_f_{p_type}_patient'.format(
                    learning_rate=learning_rate,
                    dropout_type=dropout_type_for_train, dropout_rate=dropout_rate_for_train, e=epoch + 1, month=month,
                    feature_num=selected_feature_num, p_type=p_type)
                torch.save(model.state_dict(),
                           '{CHECKPOINT_PATH}/{model_name}_state_dict.pt'.format(CHECKPOINT_PATH=CHECKPOINT_PATH,
                                                                                 model_name=model_name))
                # choose dropout type and rate with new loss function
                dropout_choice_result_path = '{output_folder}/dropout_choice_result_train_type_{dropout_type_for_train}_train_rate_{dropout_rate_for_train}_lr_{learning_rate}_e_{e}.csv'.format(
                    output_folder=output_folder,
                    dropout_type_for_train=dropout_type_for_train, dropout_rate_for_train=dropout_rate_for_train,
                    learning_rate=learning_rate, e=epoch + 1)
                for dropout_type in dropout_type_list:
                    for dropout_rate in dropout_rate_list:
                        loss_one_sample_average, test_loss, test_loss_mean = eval_model_new_loss(device, valid_loader,
                                                                                                 model,
                                                                                                 criterion,
                                                                                                 seq_length,
                                                                                                 step_look_back,
                                                                                                 ...
                                                                                                 conf, mean_value,
                                                                                                 std_value)
                        dropout_dict = {'dropout_type': dropout_type, 'dropout_rate': dropout_rate,
                                        'loss_value': loss_one_sample_average}
                        dropout_choice_df = dropout_choice_df.append(dropout_dict, ignore_index=True)
                dropout_choice_df.to_csv(dropout_choice_result_path)
